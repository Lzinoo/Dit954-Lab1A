...
public abstract class Vehicles implements Movable {
    ...
    public void wallDetection(int carHeight, int carWidth, int windowHeight, int windowWidth) {

        double currentX,currentY;
        currentX = getX();
        currentY = getY();

        if(currentX < 0 || currentX + carWidth > windowWidth) {
            turnLeft();
            turnLeft();
        }
        else if (currentY<0 || currentY + carHeight > windowHeight){
            turnLeft();
            turnLeft();
        }
        move();
